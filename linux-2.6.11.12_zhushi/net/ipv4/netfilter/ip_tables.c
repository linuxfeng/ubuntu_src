...
#define FWINV(bool,invflg) ((bool) ^ !!(ipinfo->invflags & invflg))
	...
	/*处理源和目标ip地址，这个if语句的意义是：
	到达分组的源ip地址经过掩码处理后与规则中的ip不匹配并且规则中没有包含对ip地址的取反，或者规则中包含了对匹配地址的取反，
	但到达分组的源ip与规则中的ip地址匹配，if的第一部分返回真，同样道理处理到达分组的目的ip地址。这两部分任意部分为真时，源或者目标地址不匹配。*/
	...
	/*接着处理输入和输出的接口
	for语句处理接口是否与规则中的接口匹配，
	不匹配时，ret返回非零，离开for语句后，
...
/*规则集在内核中顺序存放，具体规则的查找是遍历规则集，通过匹配给定的关键字找到目标规则；函数find_match即实现该功能*/
...
int ipt_register_table(struct ipt_table *table, const struct ipt_replace *repl)
{
	...
	/* 其他版本 memcpy(newinfo->entries, table->table->entries, table->table->size);*/
