...
int do_command(int argc, char *argv[], char **table, iptc_handle_t *handle)
{
	...
	while ((c = getopt_long(argc, argv,
	   "-A:C:D:R:I:L::M:F::Z::N:X::E:P:Vh::o:p:s:d:j:i:fbvnt:m:xc:",
		...
		switch (c) {
			 ...
			 * Command selection
			 */
		case 'A'://-A 或 -append 在所选链尾加入一条或多条规则
	...
	/*对于如果要进行(CMD_REPLACE | CMD_INSERT | CMD_DELETE | CMD_APPEND)处理来说，
	...
	/*对来源/目的地址及掩码进行拆分，它们总是以 addr/mask的形式来出现的，根据’/’前面的字符串取得地址值，
	 * 根据’/’后面的掩码位数，求得正确的掩码值，值得注意的是，同时要处理主机地址和网络地址的情况*/
	if (shostnetworkmask)
	...
	/*do_command函数最后一个参数handle，是一个指向了具体表，
	 * 如filter、nat表的句柄，这里判断，如果handle为空，则调用iptc_init，
	 * 根据table的名称，让handle指针指向相应的表的地址空间，
	 * 也就是把对应表的所有信息从内核中取出来:*/
	/* only allocate handle if we weren't called with a handle */
	/*iptc_init 被定义成宏　TC_INIT　，TC_INIT在libiptc.c */
	if (!*handle)
		*handle = iptc_init(*table);

	/*如果获取换败，将试着插入模块ip_tables，再次获取*/
	if (!*handle) {
